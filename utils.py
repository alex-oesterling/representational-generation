...
def compute_similarity(visual_features, profession_labels, profession_set, vision_encoder, vision_encoder_name='clip'):
    ...
    # Extract the correct column from profession_labels
    profession_indices = profession_labels[:, 0]  # Assuming the first column is the profession index

    for i, profession in enumerate(profession_set):
        with torch.no_grad():
            profession_idx = (profession_indices == i)  # Get boolean array for current profession
...
def _clip_extraction(encoder, dataloader, args, query=True):
    ...
    path = '/n/home07/iislasluz/representational-generation/datasets/mpr_stuffs/'
    ...
    # Use the save directory from arguments for saving the embeddings
    save_path = os.path.join(path, filename)
    os.makedirs(os.path.dirname(save_path), exist_ok=True)  # Ensure the directory exists
    
    # Save the embeddings to a file in the specified save directory
